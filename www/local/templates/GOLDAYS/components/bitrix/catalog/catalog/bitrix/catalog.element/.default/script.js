		...
		switch (this.productType)
		{
			...
			case 2://set
				if (this.product.useSlider)
				{
					...
					SliderImgs = BX.findChildren(this.product.slider.LIST, {tagName: 'span'}, true);
			...
			case 3://sku
				...
				for (i = 0; i < this.offers.length; i++)
				{
					...
					else
					{
						...
						SliderImgs = BX.findChildren(this.sliders[i].LIST, {tagName: 'span'}, true);
						if (!!SliderImgs && 0 < SliderImgs.length)
						{
							for (j = 0; j < SliderImgs.length; j++)
							{
...
window.JCCatalogElement.prototype.SetProductMainPict = function(intPict)
{
	...
	if (0 < this.product.sliderCount)
	{
		...
		if (-1 < indexPict)
		{
			...
			RowItems = BX.findChildren(this.product.slider.LIST, {tagName: 'span'}, false);
			if (!!RowItems && 0 < RowItems.length)
			{
				...
				for (i = 0; i < RowItems.length; i++)
				{
					...
					if (value === strValue)
					{
						BX.addClass(RowItems[i], 'active1');
					}
					else
					{
						BX.removeClass(RowItems[i], 'active2');
					}
				}
			}
		}
	...
	if (-1 < index)
	{
		if (0 < this.offers[index].SLIDER_COUNT)
		{
			...
			if (-1 < indexPict)
			{
				...
				RowItems = BX.findChildren(this.sliders[index].LIST, {tagName: 'span'}, false);
				if (!!RowItems && 0 < RowItems.length)
				{
					...
					for (i = 0; i < RowItems.length; i++)
					{
						...
						if (value === strValue)
						{
							BX.addClass(RowItems[i], 'active3');
						}
						else
						{
							BX.removeClass(RowItems[i], 'active4');
						}
					}
				}
			}
...
{ 
	...
	if (0 === this.errorCode)
	{
		...
		if (this.canBuy)
		{
			...
			this.obBuyBtn.innerHTML = '<img src="/bitrix/templates/GOLDAYS/images/icons/icon-round-white-in-cart.png" alt=""><div class="big">Отложить изделие</div><div class="small">для примерки</div></a>';
		}
		else
		{
...
window.JCCatalogElement.prototype.UpdateRow = function(intNumber, activeID, showID, canBuyID)
{
	...
	{
		RowItems = BX.findChildren(this.obTreeRows[intNumber].LIST, {tagName: 'span'}, false);
		if (!!RowItems && 0 < RowItems.length)
		{
			pictMode = ('PICT' === this.treeProps[intNumber].SHOW_MODE);
			countShow = showID.length;
			extShowMode = this.treeRowShowSize < countShow;
			strNewLen = (this.treeRowShowSize < countShow ? (100/countShow)+'%' : '8%');
			obData = {
				props: { className: '' },
				style: {
			...
			for (i = 0; i < RowItems.length; i++)
			{
				...
				{
					obData.props.className = (isCurrent ? 'none' : '');
				}
				else
				{
					obData.props.className = (isCurrent ? 'active8 bx_missing' : 'bx_missing');
				}
...
window.JCCatalogElement.prototype.incViewedCounter = function()
{
	...
	showDeleteButton(this.offers[this.offerNum].ID);
};

	...
	<nav class="menu-sidebar">
		...
		<?if($arItem["PROPERTY_TYPE"] == "N" ):?>
					...
					<li <? echo $kk[0]; ?> >
							...
							// инициализируем геты для нашего запроса;
							$ind_filt=$arItem["ID"];
							
							$ind_filt.="_";
							
							// тип запроса гет $get &lui[0]
							...
							$url=str_replace($del_ur,'',$url);
							endforeach;	
							// удаляем геты из масива lui v 0 специально для хит скидка новинка $url_sp
							$del_ur="&lui[438]=Y";
							$url_sp=str_replace($del_ur,'',$url);								
							$del_ur="&lui[436]=Y";
							$url_sp=str_replace($del_ur,'',$url_sp);
							$del_ur="&lui[437]=Y";
							$url_sp=str_replace($del_ur,'',$url_sp);
							
							//echo $url; 
							$url2=urldecode($url);
							// удаляем текущие геты из масива lui связанные с конкретным пунктом
							foreach($arItem["VALUES"] as $val => $ar):
							$del_ur="&lui[".$arItem['ID']."]=".$ar["VALUE"];
							$prov_id=$arItem["ID"];
							$url2=str_replace($del_ur,'',$url2);		
							endforeach;
							...
							<?}else{?>			
							<a  href='<?echo  $vopros.$url_sp."&lui[".$arItem['ID']."]=Y";?>' ><?=$arItem["NAME"]?><span></span></a>
							<?}	
							if($kk[1]==false){ ?>
							<select size="1">	
							<?$ii=1?>
							<option kuku="<?echo $vopros.$url2; ?>">Все варианты</option>
						<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?$get_pr=$_GET[$ar["CONTROL_ID"]];?>
						
							<?$url_ind_f="&lui[".$arItem["ID"]."]="?>
							
							<option iii='<? echo $ii;?>' vbrsp="<? echo "xx_".$arItem["ID"];?>" mmm="<?=$ar["VALUE"];?>" yes_no="<? if($get_pr=="Y"){echo "Y";}else{ echo "N";} ?>" kuku='<?echo $vopros.$url2.'&'.$ar["CONTROL_ID"]."=Y".$url_ind_f.$ar["VALUE"]; ?>'><?echo $ar["VALUE"];?></option>
						<?$ii++;?>
						<?endforeach;?>				
						</select>
						<?}

...
<div class="card-item-line">
<div class="item-metall"></div>
<?
if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS']) && !empty($arResult['OFFERS_PROP']))
{
foreach($arResult['OFFERS'] as $k => $item) {
$arIBlockElement = GetIBlockElement($item["ID"]);
$arResult['OFFERS'][$k]["XML_ID"] = $arIBlockElement["XML_ID"];
}
...
if ('TEXT' == $arProp['SHOW_MODE'])
		{
			...
			<?
			foreach($arResult['OFFERS'] as $v){$arSKU['sku_'.$v['PROPERTIES']['RAZMER']['VALUE_XML_ID']] = $v;};
			...
			<div style="text-align: right"><span id='clik_popap' onclick="iClickTypeNEW = 3;"  class="select-size open-sizes" style="">Выбрать размер</span> </div>
			</div>
...
?>
<div class="item-ves"></div>
<div class="card-item-line">
<div class="item-inserts"></div>
</div>
...
if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
{
    ...
    if($mm==true){?>
    <div id='perexvat1' >
    <a href="javascript:void(0);" onclick="iClickTypeNEW = 1;" class="<? echo $buyBtnClass." select-size open-sizes"; ?>" >
...
<? if($mm==true){?>
    <div id='perexvat1_1' >
    	...
    	<a href="javascript:void(0);"  onclick="iClickTypeNEW = 2;" class="button button-black inline  open-sizes"  >
    ...
    <div id='perexvat2_1' style='display:none'>
			...
			<a href="javascript:void(0);" class="button button-black inline <? echo $buyBtnClass; ?>"  onclick='tryBuy();'>
	<img src="<?=SITE_TEMPLATE_PATH?>/images/icons/icon-round-white-try.png" alt="">Примерить сразу
	</a>
...
<script>
$('#obrotchik .onlickdata').on('click', function(){
console.log(arSKU);
	var kk = "sku_"+$(this).data('key');
	var r = parseFloat(arSKU[kk].MIN_PRICE.DISCOUNT_VALUE);
	var v = parseFloat(arSKU[kk].DISPLAY_PROPERTIES.VES.VALUE.replace(',','.'));
	
	if(!$.isEmptyObject(arSKU[kk].DISPLAY_PROPERTIES.MATERIAL))$(".item-metall").html('<strong>Материал:</strong> '+arSKU[kk].DISPLAY_PROPERTIES.MATERIAL.VALUE+', '+arSKU[kk].DISPLAY_PROPERTIES.PROBA.VALUE+' проба'); else $(".item-metall").html('');
	if(!$.isEmptyObject(arSKU[kk].DISPLAY_PROPERTIES.VSTAVKA))$(".item-inserts").html('<strong>Вставка:</strong> '+arSKU[kk].DISPLAY_PROPERTIES.VSTAVKA.VALUE); else $(".item-inserts").html('');
	if(v)$(".item-for-weight").html('<span id="cena_gramm">'+parseInt(r/v)+' P</span> за грамм'); else $(".item-for-weight").html('');
	if(v)$(".item-ves").html('<strong>Вес:</strong> '+v); else $(".item-ves").html('');
});
...
var arSKU = <? echo CUtil::PhpToJSObject($arSKU, false, true); ?>;
console.log(arSKU);
...
var iClickTypeNEW = 0; // был установлен ноль незнаю зачем понменял на 2 заработало
...
$(window).on('load', function(){
    ...
    $("#obrotchik span").each(function(){
        ...
        if( (ff) && (bb)) {
        	...
        	if(flag==0){
				var key = "sku_"+ff;
				if(!$.isEmptyObject(arSKU[key].DISPLAY_PROPERTIES.MATERIAL))$(".item-metall").html('<strong>Материал:</strong> '+arSKU[key].DISPLAY_PROPERTIES.MATERIAL.VALUE+', '+arSKU[key].DISPLAY_PROPERTIES.PROBA.VALUE+' проба'); else $(".item-metall").html('');
				if(!$.isEmptyObject(arSKU[key].DISPLAY_PROPERTIES.VSTAVKA))$(".item-inserts").html('<strong>Вставка:</strong> '+arSKU[key].DISPLAY_PROPERTIES.VSTAVKA.VALUE); else $(".item-inserts").html('');
				//$(this).click();
        		 $(this).css('border-left','solid 1px #fff');
                    flag=1;
                   }
...
function SizeClick(kk, ff) {
	...
	//$("#perexvat2 a").click();
        //nos.CheckAvailTovars();
        $('#size-select').fadeOut(200).siblings('#overlay').fadeOut(200);
    }

    if (iClickTypeNEW == 1) {
	$.ajax({
	  url: "?action=BUY&id="+arSKU["sku_"+ff]["ID"],
	  success: function(){
    ...
    if (iClickTypeNEW == 2) {
	$.ajax({
	  url: "/bitrix/components/ipg/order.confirm/ajax.php?action=order&list[0][price]="+arSKU["sku_"+ff]["MIN_PRICE"]["DISCOUNT_VALUE_VAT"]+"&list[0][xmlid]="+arSKU["sku_"+ff]["XML_ID"],
...
<script>
function tryBuy() {
	var ff=$("#obrotchik span.active").attr('ff');
	$.ajax({
	  url: "/bitrix/components/ipg/order.confirm/ajax.php?action=order&list[0][price]="+arSKU["sku_"+ff]["MIN_PRICE"]["DISCOUNT_VALUE_VAT"]+"&list[0][xmlid]="+arSKU["sku_"+ff]["XML_ID"],
	  success: function(data){
	    if(data=="true"){
